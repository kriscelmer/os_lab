...
openstack subnet create --network provider-net --allocation-pool start=192.168.233.100,end=192.168.233.127 --dns-nameserver 10.0.0.11 --gateway 192.168.233.2 --subnet-range 192.168.233.0/24 provider-net-subnet
```
...
openstack subnet create --network demo-net --subnet-range 10.10.10.0/24 --dns-nameserver 10.0.0.11 demo-subnet
...
**6.3 Configure and test Designate:** Since we enabled Designate, let’s set it up:
...
Next, associate `provider-net` network with `example.test.` domain so that when instances are launched on that network, Neutron notifies Designate to publish the DNS records:

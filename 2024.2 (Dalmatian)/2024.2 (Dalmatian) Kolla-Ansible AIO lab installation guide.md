...
Start the `os-dalmatian-aio` VM and perform a **minimized** installation of Ubuntu Server 24.04 on the VM, use `English` as main language. Install system on 100GB disk (!), LVM on root disk is not required for the LAB purposes. Configure only first Network Interface with DHCP only, all other Network Interface will be configured later. Configure default user with **`openstack`** username and password of your choice. The course assumes this username in exercises. Set hostname to `controller`. Check `Install OpenSSH server` option, it's required for the LAB. 
...
**2.6 Prepare the Second Disk for Cinder LVM:** The 120GB disk (`/dev/sdb` in the VM) will be used for Cinder volumes gropups `cinder-volumes` and `cinder-volumes2` via LVM. Set up LVM:
...
This uses the Kolla-Ansible utility to download necessary Ansible roles (you may see it installing roles for **ceph**, etc.).
...
Create a project **demo-tenant** for lab exercises and user **demo** in this project:
...
User's password is set to `openstack`, which is OK for the demo Lab. Let's create an **openrc** file the user `demo` in project `demo-tenant`:
...
Create a **flavor** as **admin** user (defore switching to **demo** user credentials), and share the `example.test.` zone with demo project:

```bash
# Create a flavor
openstack flavor create --ram 256 --disk 1 --vcpus 1 m1.tiny

# Share the Designate zone example.test. with project demo-tenant using its ID
...
You should see an A record for `test-vm`. Also, from the VM’s console, try querying the DNS:
...
```bash
ping -c 3 test-fip.example.test
```
...
Test **Designate** configuration by using FQDN inside of the `test-vm` Instance, run this command on the Instance prompt:

```bash
ping -c 3 google.com
```

(Exit the Instance before running subsequent commands.)
...
**6.6 Central logging:** We have enabled Kolla Ansible Central Logging feature which installs and deploys OpenSearch for log aggregation, analysis and visualisation.
OpenSearch web console can be accessed from Windows host by typing `10.0.0.11:5601` into browser. Default username is `opensearch` and password can get retrieved with:
...
**6.7 Telemetry (Ceilometer/Gnocchi/Aodh):** The telemetry services (if enabled) will collect metrics and allow setting alarms:

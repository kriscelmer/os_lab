# OpenStack 2024.2 All-in-One Installation with Kolla-Ansible on Ubuntu 22.04 LTS

This guide walks through deploying OpenStack **2024.2** (stable/Dalmatian release) on a single Ubuntu 22.04 VM using **Kolla-Ansible**. We will set up an All-In-One (AIO) environment in VirtualBox with one controller/compute node. All OpenStack services will run in Docker containers orchestrated by Kolla-Ansible. The configuration includes a Cinder LVM backend for block storage (with a dedicated disk) and Designate for DNS service with a local Bind9 server.

## 1. Virtual Machine Setup and Networking

Before installation, create a VirtualBox VM with the following specs and network layout:

- **Virtual Machine name:** `os-dalmatian-aio`, if you change it mind the commands below, as they may reference this name
- **vCPU & RAM**: 16 vCPUs, 16 GB RAM  
- **Storage**:  
  - Disk1: 100 GB (for OS and OpenStack containers)  
  - Disk2: 120 GB (for Cinder LVM volumes)  
- **Network Interfaces**:  
  ...
  - `Adapter 2`: Attach to a **Host-Only Network** (VirtualBox Host Network) and assign it a static IP (e.g. **10.0.0.11/24**). This will be the OpenStack **management network** interface, used for Horizon (dashboard) access and internal API communication. Create the Host Network in VirtualBox Network Manager window, if necessary: Host-only Networks, Configure Adapter Manually, IPv4 Address 10.0.0.1, IPv4 Network Mask 255.255.255.0, DHCP Server - uncheck Enable Server.
  - `Adapter 3`: Attach to a **VirtualBox NAT Network** (or an internal network) configured with subnet **203.0.113.0/24**. This will serve as the Neutron **provider network** for external (Floating IP) connectivity. In `Advanced` settings section enable Promiscuous Mode to `Allow All`. Do **not** configure an IP on `enp0s9` inside the VM (it will be managed by Neutron). Create the NAT Network in VirtualBox Network Manager window, if necessary: NAT Networks, General Options, Name "provider", IPv4 Prefix 203.0.113.0/24, uncheck Enable DHCP and Enable IPv6. 
...
Perform a **minimal** installation of Ubuntu Server 22.04 on the VM, use `English` as main language. Install system on 100GB disk (!), LVM on root disk is not required for the LAB purposes. Configure only first Network Interface with DHCP only, all other Network Interface will be configured later. Configure default user with **`openstack`** username and password of your choice. The course assumes this username in exercises. Set hostname to `controller`. Check `Install OpenSSH server` option, it's required for the LAB. After installation and reboot configure the system as follows:
...
sudo apt install -y python3-dev python3-venv libffi-dev gcc libssl-dev libdbus-glib-1-dev vim nettools
```
...
Open `/etc/kolla/globals.yml` in your editor and set the following parameters to match our environment. We will disable high availability (since this is a one-node deployment) and enable the required OpenStack services. Below is a summary of key settings to change or add:
...
- **Other Misc Settings:**  
  You can review `globals.yml` for any other defaults. For a single-node, you might also set:
  ```yaml
  # Use the same IP for internal/external VIP if haproxy was enabled (not used since we disabled HA)
  kolla_internal_vip_address: "10.0.0.11"
  ```
  The above is not strictly needed with HAProxy disabled, but it doesn’t hurt to define it to the management IP. Also, ensure `docker_registry` and related settings are left as default (so images will be pulled from Docker Hub/Quay as needed). Since this is a lab, we are using the upstream container registry.

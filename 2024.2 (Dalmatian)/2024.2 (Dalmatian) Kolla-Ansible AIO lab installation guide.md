...
- **Network**:  
  - `Adapter 1`: Attach to **NAT** (DHCP enabled). This provides outbound connectivity for package installation and container pulls.  
  ...
  - `Adapter 3`: Attach to **NAT** (DHCP disabled). This will be used for provider network in OpenStack.  
  - For each Adapter select **Paravirtualized Network (virtio-net)** as **Adapter Type**

**Networking details:** In this setup, `ens33` (10.0.0.11) is an isolated network between your host and VM (for SSH and accessing Horizon), and `ens34` is a provider network that instances will attach to for external access.

Check all **Virtualization engine** options in Virtual Machine Settings of `os-dalmatian-aio`, Hardware->Processor
...
openstack subnet create --network provider-net --allocation-pool start=203.0.113.100,end=203.0.113.200 --dns-nameserver 8.8.8.8 --gateway 192.168.233.2 --subnet-range 203.0.113.0/24 provider-net-subnet
```
